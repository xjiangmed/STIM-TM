...
def bipartite_soft_matching(
    ...
    with torch.no_grad():
        ...
        if merging_type == 'frame' and frame_average:  # aggregate frames based on patch voting
            n = metric.size(-3)  # number of patches K
            node_idx, _ = node_idx.mode(dim=-2, keepdim=True) 
            node_idx = node_idx.repeat(1, n, 1) #(B,N,T//2) 

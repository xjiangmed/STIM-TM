...
class PhaseDataset_AutoLaparo(Dataset):
    ...
    def _video_batch_loader(self, duration, indice, video_id, index, cut_black):
        ...
        frame_sample_rate = self.frame_sample_rate #根据 frame_sample_rate 决定采帧间隔：
        sampled_list = []
        frame_id_list = []
        for i, _ in enumerate(range(0, self.clip_len)):
            frame_id = indice
            frame_id_list.append(frame_id)
            if self.frame_sample_rate == -1: #每次随机1-5的间隔
                frame_sample_rate = random.randint(1, 5)
            elif self.frame_sample_rate == 0: #使用2^i的指数间隔
                frame_sample_rate = 2**i
            elif self.frame_sample_rate == -2: #使用1,2,4,6...的递增间隔
        ...
        # print('sampled_list:', sampled_list) #[1604,1608,1612,...,1664]
        sampled_image_list = []
        sampled_label_list = []
        image_name_list = []
